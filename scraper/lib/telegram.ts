...
export class TelegramNotifier {
  ...
  constructor() {
    ...
    if (botToken && chatId) {
      ...
      logger.debug('Telegram notifications disabled - missing TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID')
    }
  }
  ...
  private formatBatchMessage(results: ScrapingResult[]): string {
    ...
    const totalDiscounts = successful.reduce((sum, r) => sum + (r.discountsFound || 0), 0)
