...
describe("Payment Method Wizard Categories", () => {
  test("should categorize all payment methods", () => {
    ...
    const missingMethods = allPaymentMethods.filter(method => 
      !categorizedMethods.has(method as PaymentMethod)
    );
    ...
    if (missingMethods.length > 0) {
      console.warn("⚠️ Missing payment methods in wizard categories:", missingMethods);
    }
  });
  ...
  test("should include all known digital wallets", () => {
    ...
    const missingDigitalWallets = knownDigitalWallets.filter(wallet => 
      !CATEGORY_FILTERS.digitalWallets(wallet as PaymentMethod)
    );
    ...
    if (missingDigitalWallets.length > 0) {
      console.warn("⚠️ Missing digital wallets in wizard filters:", missingDigitalWallets);
    }
  });

  test("should categorize banks correctly", () => {
    const bankMethods = BANKS_OR_WALLETS.filter(method => 
      method.startsWith("Banco") || method === "Sidecreer" || method === "BanCo (Banco de Corrientes)"
    );
  ...
  test("should categorize cards correctly", () => {
    const cardMethods = [...BANKS_OR_WALLETS, ...PAYMENT_RAILS].filter(method => 
      method.startsWith("Tarjeta") || ["MODO", "Tarjeta American Express"].includes(method)
    );
  ...
  test("should not double-categorize payment methods", () => {
    ...
    allPaymentMethods.forEach(method => {
      ...
      if (CATEGORY_FILTERS.banks(method as PaymentMethod)) categorizedIn.push("banks");
      if (CATEGORY_FILTERS.cards(method as PaymentMethod)) categorizedIn.push("cards");
      if (CATEGORY_FILTERS.digitalWallets(method as PaymentMethod)) categorizedIn.push("digitalWallets");
